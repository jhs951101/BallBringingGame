...
void Computer_smart(int &marbles) {  // 컴퓨터가 구슬을 가져갈 차례일 때 실행하는 함수(스마트 모드용)
	...
	if (marbles > 1) {
		cout << endl;
		cout << "There are " << marbles << " marbles in the pile." << endl << endl;  // 구슬 갯수 출력
	}
}
...
int smart_mode = 2; //Random(1, 2);  // (전역 변수) smart_mode: 1-일반 모드, 2-스마트 모드
...
int main() {
	...
	do {
		...
		cout << endl;
		cout << "There are " << marbles << " marbles in the pile." << endl << endl;  // (한번만 사용됨)
